...
class Nlp(object):
    ...
    def summarize(self, url='', title='', text='', max_sents=5):
        if not text or not title or max_sents <= 0:
            return []
        summaries = []
        sentences = self.split_sentences(text)
        keys = self.keywords(text)
        titleWords = self.split_words(title)
        # Score sentences, and use the top 5 or max_sents sentences
        ranks = self.score(sentences, titleWords, keys).most_common(max_sents)
        for rank in ranks:
    ...
    def score(self, sentences, titleWords, keywords):
    ...
    def keywords(self, text):
        ...
        if text:
            ...
            for k in keywords:
                articleScore = (keywords[k] * 1.0 / max(num_words, 1) * 100)
                keywords[k] = round(articleScore * 1.5 + 1, 3)
